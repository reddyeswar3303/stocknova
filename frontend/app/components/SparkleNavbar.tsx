...
const SparkleNavbar: React.FC<SparkleNavbarProps> = ({
  ...
  const createSVG = (element: HTMLDivElement) => {
    // ... SVG drawing logic remains unchanged ...
    element.innerHTML = `
    <svg viewBox="0 0 116 5" preserveAspectRatio="none" class="beam" style="position: absolute; right: 0; top: 50%; transform: translateY(-50%); width: var(--active-element-width); overflow: visible; height: 5px; filter: blur(0.5px) drop-shadow(2px 0px 8px ${color}40) drop-shadow(1px 0px 2px ${color}80);">
    ...
    <div class="strike" style="position: absolute; right: 0; top: 50%; transform: translateY(-50%); width: var(--active-element-width); padding: 24px 0; -webkit-mask-image: linear-gradient(to right, transparent calc(0% + var(--active-element-mask-position)), black calc(15% + var(--active-element-mask-position)), black 80%, transparent);">
      <svg viewBox="0 0 114 12" preserveAspectRatio="none" style="display: block; overflow: visible; height: 12px; width: calc(var(--active-element-width) * 2); transform: translate(var(--active-element-strike-x), 30%) scale(var(--active-element-scale-x), var(--active-element-scale-y));">
